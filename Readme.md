...
- 矿车滑翔功能：
  ...
  - `滑翔`：
    ...
    - 撞击方块时会退出滑翔状态并触发`坠机`
    - 滑翔状态的矿车在竖向速率大于`-0.2`时尝试进入`降落`状态
      - 下方是空气则拒绝降落
      - 本格或下方是铁轨拒绝进入`降落`状态，而是直接退出滑翔状态
      - 其他：进入`降落`状态
    - 滑翔状态的矿车没有进入降落状态时会尝试退出滑翔状态
      - 下方是空气继续滑翔
      - 本格或下方是铁轨则退出滑翔状态
...
- 矿车航行功能：
  - 仅对玩家正在乘坐的矿车生效
  - 在`降落`状态下接近/触碰水面可进入`航行`状态
  - `航行`：特殊的降落状态
    - 具有全部的滑翔特性
    - 操纵方式同飞行
    - 航行状态下，拥有降落阻力和可能的水阻力，因此阻力较大，不一定能达到极速
    - 可以完全没入水中
