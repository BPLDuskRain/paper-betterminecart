...
- 由火焰弹生成的温泉会在开启持久化后保存温泉方块到指定的数据库，开机读取，而矿车的不会
  - 直接保存每一个温泉方块，可能会对硬盘、加载/卸载速度造成一定影响，请注意数量
## 侵入物品
- 矿车 `MINECART` 实体
  - 玩家乘坐时
...
+ 载具碰撞监听
  ...
  + 碰撞方块
    ...
    + 无重力（滑翔/飞行/航行）
      ...
      + 爆炸
        + 有`5刻`延迟
        + 对可被伤害乘客实体造成和单向最大速率正相关的伤害并给予`100刻`点燃
        + 弹出乘客
        + 产生模拟爆炸的粒子和音效
        + 创建`1200刻`的临时温泉
          + 产生模拟加热的粒子和音效
